<?php
  ...
  function getAllImagesWithAlgortihmsWithSpecificTag($conn, $tag) {
    $images = array();

    $sql = "SELECT DISTINCT image.content, image.id FROM image JOIN algorithm alg ON image.id = alg.imageId WHERE alg.note LIKE '" . $tag ."%' ORDER BY alg.note";
	  $result = $conn -> query($sql);
